...
import com.cognifide.knotx.server.api.RepositoryRequest;
import com.cognifide.knotx.server.api.RepositoryResponse;
import com.cognifide.knotx.server.api.TemplateEngineRequest;
import com.cognifide.knotx.server.util.RepositoryRequestCodec;
import com.cognifide.knotx.server.util.RepositoryResponseCodec;
import com.cognifide.knotx.server.util.TemplateEngineRequestCodec;
...
public class KnotxServerVerticle extends AbstractVerticle {
    ...
    public void start(Future<Void> fut) throws IOException, URISyntaxException {
        ...
        httpServer.requestHandler(
                request -> eventBus.<RepositoryResponse>sendObservable(repositoryAddress, createRepositoryRequest(request))
                        .doOnNext(this::traceMessage)
                        .subscribe(
                                reply -> {
                                    RepositoryResponse repository = reply.body();
                                    if (repository.isSuccess()) {
                                        eventBus.sendObservable(engineAddress, createEngineRequest(repository, request))
                                                .subscribe(
                                                        result -> {
