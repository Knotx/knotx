...
public final class AdapterProxyFactory {

  private AdapterProxyFactory() {
    // hidden
  }

  public static AdapterProxy createProxy(Vertx vertx, Optional<DeliveryOptions> deliveryOptions, String address) {
    return deliveryOptions
        .map(options -> AdapterProxy.createProxyWithOptions(vertx, address, options))
        .orElse(AdapterProxy.createProxy(vertx, address));
  }
