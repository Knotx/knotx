...
| `clientDestination`         | `JsonObject`                        | &#10004;       | Allows to specify HTTP repository connection details using **scheme**, **domain**, **port** values (<scheme>://<domain>:<port>). Additionally, it's possible to specify override of the host header - **host** field |
...
| `host`        | `String`  |                | Override of the host header used in that communication. If set, this is the value that will be effectively send. |
...
"clientDestination": {
  ...
  "port": 443,
  "host": "specific.repo.resolution.domain"
}
```
